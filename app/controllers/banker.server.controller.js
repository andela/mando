...
exports.getAllAccounts = function (req, res) {
  subledger.organization(org_id).book(book_id).account().get({
    'description': 'USD'
...
exports.getUniqueAccount = function (req, res) {
  ...
  subledger.organization(org_id).book(book_id).account(account_id).get({
    'description': 'USD'
  }, function (error, apiRes) {
    if (error) {
      return res.json(error);
...
exports.creditUserAccount = function (action, amount, initiatorAccount, recipientAccount, title, cb) {
  ...
  subledger.organization(org_id).book(book_id).journalEntry().createAndPost({
